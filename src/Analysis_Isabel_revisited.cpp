      ...
      void run(const HEPUtils::Event* event){ 
        ...
        if (nPhotons == 2) {
          ...
          if (
            nElectrons == 0 &&
            nMuons == 0 &&
            signalPhotons.at(0)->pT() > 35. &&
            Diphoton_invm > 95. &&
            Diphoton_invm < 160. &&
            signalPhotons.at(1)->pT() / Diphoton_invm > 0.2 &&
            nBJets == 2 &&
            nJets == 2 // This is modified from the source
          ) 
            {
        ...
        if (canidate_event){
          ...
          const bool met_SR1 = (met <= 100.);
          const bool met_SR2 = (met > 100.);
          const bool m_diphoton = (Diphoton_invm > 120. && Diphoton_invm < 130.);
          const bool SR1h_m_bb = (bb_invm > 100. && bb_invm < 140.);
          const bool SR1Z_m_bb = (bb_invm > 60. && bb_invm < 100.);
          const bool SR2_m_bb = (bb_invm > 35. && bb_invm <= 145.);
          const bool ratio_04 = (ratio_m_pT > 0.4);
          const bool ratio_02 = (ratio_m_pT > 0.2);
          const bool diphoton_pt = (diphoton_pT > 90.);
          ...
          if (met_SR1 && m_diphoton && SR1h_m_bb && ratio_04) _counters.at("SR1h_pm_04").add_event(event);
          if (met_SR1 && m_diphoton && SR1h_m_bb && ratio_04 && diphoton_pt) _counters.at("SR1h_p_diphoton_90").add_event(event);
          ...
          if (met_SR1 && m_diphoton && SR1Z_m_bb && ratio_04) _counters.at("SR1Z_pm_04").add_event(event);
          if (met_SR1 && m_diphoton && SR1Z_m_bb && ratio_04 && diphoton_pt) _counters.at("SR1Z_p_diphoton_90").add_event(event);
          if (met_SR1 && m_diphoton && SR1Z_m_bb && ratio_04 && diphoton_pt) _counters.at("SR1Z").add_event(event);

          // SR2 cutflow
          if (met_SR2) _counters.at("SR2_ETmiss_100").add_event(event);
          if (met_SR2 && m_diphoton) _counters.at("SR2_m_diphoton").add_event(event);
          if (met_SR2 && m_diphoton && ratio_02 && SR2_m_bb) _counters.at("SR2_m_bb").add_event(event);
          if (met_SR2 && m_diphoton && ratio_02 && SR2_m_bb) _counters.at("SR2").add_event(event);
